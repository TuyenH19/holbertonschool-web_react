...
describe('App', () => {
  ...
  test('App component should render the CourseList table when state.user.isLoggedIn is true', () => {
    ...
    if (fiber && fiber.stateNode) {
      // Manually set the state to logged in
  ...
  test('Verify UI updates when user logs out (state changes)', () => {
    const { container } = render(<App />);
    ...
    // UI should now show Login form again
    expect(screen.getByText(/login to access the full dashboard/i)).toBeInTheDocument();
  ...
  test('Clicking on a notification item removes it from the list and logs to console', () => {
