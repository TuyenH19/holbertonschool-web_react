...
test('it should render with urgent type and have correct data attribute', () => {
  const mockMarkAsRead = jest.fn();
  ...
  const liElement = container.querySelector('[data-notification-type="urgent"]');
  expect(liElement).toBeInTheDocument();
  expect(liElement.textContent).toBe('Urgent notification');
});

test('it should render with default type and have correct data attribute', () => {
  const mockMarkAsRead = jest.fn();
  const { container } = render(
    <NotificationItem
      id={2}
  ...
  const liElement = container.querySelector('[data-notification-type="default"]');
  expect(liElement).toBeInTheDocument();
  expect(liElement.textContent).toBe('Default notification');
});
